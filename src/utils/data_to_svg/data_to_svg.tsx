...
function phi_n(i: string, fi: string, dx: number, dy: number, h: number, ymin: number, ynorm: number) {
    let x = Number(i)
    let fx = Number(fi)
    return Math.round(dx * x).toString() + " " + Math.round((h - dy * (fx - ymin)) / ynorm).toString()
}


export async function svgPathMaker_normalized(ticker: string, from: string, to: string, width: number, height: number, step: number) {
    let uri = `https://web.api.six-group.com/api/findata/v1/listings/marketData/eodTimeseries?scheme=TICKER_BC&ids=${ticker}&from=${from}`
    return query_six(uri).then(data => {
        let ts = data.data.listings[0].marketData.eodTimeseries
        let dx = width / ts.length
        ...
        let dy = height / (max - min)
        console.log(dx, dy)
        let ynorm = Number(ts[0].close)
        let res = "M " + phi_n("0", ts[0].close, dx, dy, height, min, ynorm) + " "
        for (let i = 0; i < ts.length; i += step) {
            let j = i.toString()
            if (ts[j].close) {
                res += "L " + phi_n(j, ts[j].close, dx, dy, height, min, ynorm) + " "
            }
        }
        console.log(data["listings"])
        return res
